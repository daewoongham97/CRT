...
# original_df: inputted dataframe where it contains the original dataframe with the left and right profiles where columns indexed with _2 refer to the right profiles
get_AMCEdf = function(original__df) {
  second_idx = which(sapply(strsplit(colnames(original__df)[-ncol(original__df)], "_"), "[[", 2) == "2")
  
  y_1 = original__df$y
  first = original__df[, -second_idx]
  second = original__df[, second_idx]
  y_2 = 1-y_1
...
# Helper function to augment original data matrix to create D^c matrix as shown in Appendix Table 4
augment_df = function(x_df) {
  n = nrow(x_df)
  first = grep(".+_1", colnames(x_df))
  second = grep(".+_2", colnames(x_df))
  empty_df = matrix(0, nrow = n, ncol = ncol(x_df))
  y_new = 1-x_df$y
  for (i in 1:length(first)) {
    empty_df[, first[i]] = x_df[, second[i]]
    empty_df[, second[i]] = x_df[, first[i]]
  }
  empty_df[, ncol(x_df)] = y_new
  second_df = data.frame(empty_df)
  colnames(second_df) = colnames(x_df)
  final_df = rbind(x_df, second_df)
  return(final_df)
}
...
simulate_dat = function(n, main_x, main_z, num_z, num_within, num_between, within_signal, between_signal, z_interact = rep(0.05, 15), s = sample(1:1000, size = 1)) {
  set.seed(s)
  x1 = sample(c(-0.5, 0.5), size = n, replace = TRUE)
  x2 = sample(c(-0.5, 0.5), size = n, replace = TRUE)
  
  z_samp1 = sample(c(-0.5, 0.5), size = n, replace = TRUE)
  z_samp2 = sample(c(-0.5, 0.5), size = n, replace = TRUE)
  z_df = list()
  for (i in 1:num_z) {
  ...
  y_probs = x_main + Reduce("+", z_signals) + Reduce("+", within_signals) + Reduce("+", between_signals) + Reduce("+", z_interacts) + 0.5 
