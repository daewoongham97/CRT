...
# Runtime: Approximately 2000 minutes/33 hours with 200 cores of parallel computing - for each simulation parameter calculating 1000 p-values takes 500 minutes (4 simulation parameters)
...
within_signal = rep(int_signal, num_within)*sqrt(4/3) # Stronger Interaction

num_between = num_int[param_num]/2
between_signal = rep(-int_signal, num_between)*sqrt(2/3) # Weaker Interaction
...
p_val_hetero = (length(which(e_4 >= obs_hier_long)) + 1)/(B + 1)

final_df = data.frame(method = c("CRT_hetero"), pval = c(p_val_hetero))

file_name = paste0("data/Appendix_simulations/Fig5_rightplot_results/pval", param_num, "_", loop_num, ".csv")
